...
export const generateAiReport = async ({ month }: GenerateAiReportSchema) => {
  ...
  //pegar as transações do mês recebidos
  ...
  // mandar as transsacções para o ChatGPT e pedir para ele gerar relatório
  const content = `Gere um relatório com insights sobre as minhas finanças, com dicas e orientações de como melhorar 
  minha vida financeira. As transações estão didividas por ponto e vírgula. A estrutura de cada uma é {DATA}--{TIPO}--
  {VALOR}--{CATEGORIA}. São eles:
  ${transactions
    .map(
      (transaction) =>
        `${transaction.updatedAt.toLocaleString("pt-BR")}-R$${transaction.amount}-${transaction.type}-${transaction.category}`,
    )
    .join(";")}`;
  ...
  // pegar o relatório gerado chatGPT e retornar para o usuário
